...
## Authentication Flow
Students log in with email/ID (OTP or simple login). Auth state is managed via Firebase Auth and React context.

## Order Status Flow
`Placed → Accepted → Preparing → Ready → Collected`

## Example Firebase Order Document
```
{
  id: 'orderId123',
  studentId: 'student123',
  items: [
    { itemId: '1', name: 'Samosa', quantity: 2, price: 20 }
  ],
  statusTimeline: [
  ...
  estimatedPickup: ...,
  notifications: [
    { type: 'in-app', message: 'Order accepted', timestamp: ... }
  ]
}
```
...
- Deploy `/dist` to Firebase Hosting or Netlify

---

See `/src/services/orders.js` for API and schema details.
See `/src/pages/Home.jsx` for a sample menu screen skeleton.
